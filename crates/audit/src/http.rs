//! Signed audit-webhook sink — a thin [`AuditSink`] adapter over
//! [`siphon_ai_http::RetryingPoster`], the same transport the
//! lifecycle-webhook and CDR sinks use.
//!
...
//! label. The signature is what makes the stream *tamper-evident*: a
//! SIEM verifying `X-SiphonAI-Signature` detects any in-flight edit.
...
pub struct HttpSinkConfig {
    ...
    /// unsigned (strongly discouraged for an audit stream).
    #[serde(default)]
...
/// HTTP audit-event sink. Cheap to clone (the inner [`RetryingPoster`]
/// holds a reference-counted client).
#[derive(Debug, Clone)]
pub struct HttpSink {
