...
| `user_agent` | string           | `siphon-ai/<version>` | Product token for the `Server` header on responses this daemon sends — set it to brand the node (`"Acme Voice Bridge/2.1"`), leave it unset for the product and its own version (`siphon-ai/<version>`, e.g. `siphon-ai/0.49.7`). **Requests carry `sip-uac/<version>` regardless of this setting**: the SIP stack stamps its own crate constant on `User-Agent` and takes no configured value, so REGISTER, INVITE, REFER and BYE name the stack rather than the product until that is fixed upstream ([#539](https://github.com/thevoiceguy/siphon-ai/issues/539)). Until the release carrying #539 this key was documented but wired to nothing at all: responses read `siphon-rs/0.1.0`, and `sip-uas/<version>` after the siphon-rs v2026.08.19 bump. Restart-required. |
