...
| `siphon_ai_idle_keepalive_frames_total` | counter   | —                                     | Caller-leg 20 ms keepalive frames emitted while the WS server is silent (`[bridge].idle_keepalive`, our fix for upstream issue [#610](https://github.com/thevoiceguy/siphon-ai/issues/610)). `"silence"` counts digital-silence frames, `"comfort_noise"` counts low-level comfort-noise frames; every emitted frame counts, zeros included — the counter answers "is the keepalive flowing", while "is it audible" is the config, not the metric. Stays at 0 with the default `"off"`, and pauses whenever anything else owns the caller's ear (playout in progress, MOH/park/hold/announcement, barge-in arbitration, mute, conference room, peer hold, WS drop). |
