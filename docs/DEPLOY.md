...
| `siphon_ai_notify_total`                | counter   | `result=accepted\|bad_event\|bad_request` | Inbound NOTIFYs answered (#357). `accepted` = `Event: refer` (post-REFER transfer progress, RFC 3515) → `200 OK`, dropped without WS surfacing; `bad_event` = an event package we don't support → `489 Bad Event`; `bad_request` = no `Event` header → `400`. A rising `bad_event` usually means a peer expects a subscription package (dialog, message-summary, …) SiphonAI doesn't implement. |
