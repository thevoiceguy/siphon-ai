...
8. ✅ **Wiring path for RR data into siphon-ai's `RtpStatsTracker`** — **resolved 2026-05-26 during the W1 PR session**. forge-engine emits a **new** `ForgeEvent::RtcpReportReceived { call_id, jitter_ms, packet_loss_ratio, rtt_ms: Option<f32>, timestamp }` variant on every received RR. siphon-ai's tap consumes it and feeds `RtpStatsTracker`. Rejected (A) reusing `QualityDegraded` for per-RR emission — the name lies, and a future threshold-emitter would need a sibling variant anyway. Rejected (C) a snapshot accessor on `MediaSession` — fragments the telemetry path; the event bus is already how everything else flows. Producer-side wiring lives in forge-engine's `forwarding.rs` RR-block loop, single new call site.
9. ✅ **0.3.0 scope for jitter + packet_loss** — **resolved 2026-05-26**. Wire jitter and packet_loss alongside the new RTT field, even though they were nominally in scope before. Pre-existing 0.2.0 gap: forge-engine receives the RR data and pushes it to HEP + metrics but never publishes a `ForgeEvent`, so siphon-ai's `rtp_stats` WS event has been silently emitting all-`null` fields since 0.2.0. The new `RtcpReportReceived` emitter closes all three at one call site.
10. ✅ **Outbound SR origination in forge-engine** — **deferred to 0.3.1**. The B2BUA-terminator mode forge-engine runs in for siphon-ai owns the SIP-side RTP socket but never originates its own SRs. Without our-side SRs, `RttTracker::record_outgoing_sr` has no caller and `rtcp_rtt_ms` is `None`. Adding the periodic SR emitter (RFC 3550 §6.2 bandwidth rules + SSRC bookkeeping) is its own chunk of work; 0.3.0 ships with `rtcp_rtt_ms: null` per the §7 slip plan. The forge-media RTT PR lays the `RttTracker` primitive so 0.3.1 wiring is a small follow-up.
