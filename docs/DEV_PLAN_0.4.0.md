...
1. **Identity header parsing + PASSporT verification.**
   - **Upstream**: siphon-rs PR adding `sip-identity` crate. Parses RFC 8224 `Identity:` header (`<base64>;info=<x5u>;alg=ES256;ppt=shaken`), decodes the PASSporT JWT (RFC 8225) — header + payload + signature — verifies the ES256 signature, and validates the signing certificate's chain against supplied STI-PA trust anchors (`Passport::verify_with_chain`). **Cert fetching is deliberately *not* upstream**: `x5u` retrieval is async network I/O with a TTL cache, owned by the application layer that has the runtime and the cache-TTL config (architecture decision; the stack crate stays I/O-free).
   - **siphon-ai**: A new `crates/stir-shaken` verifier-service crate owns the application half — the `x5u` HTTPS fetch (redirect-free, size/time-capped), the TTL cert cache keyed by URL, trust-anchor loading, the `orig`/`dest` claim ↔ SIP From/To E.164 checks, and verdict orchestration: `Verifier::verify(identity, from, to) → VerificationResult`. The verifier runs as a step in the INVITE accept path, before route matching, and its result is stored on per-call state for downstream surfacing. **No** policy enforcement at this layer — surfacing + observability only; the policy gate (item 3) is the layer that decides what to do with the verdict.
