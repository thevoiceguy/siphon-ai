...
| Correlated logs (OTLP)              | With `[observability.otlp.logs]` (0.51.0), the daemon's log records ship over OTLP carrying the `trace_id`/`span_id` of the span they were emitted inside — so opening a call's trace in Tempo / Jaeger / Cloud Trace shows the lines that explain each span, instead of sending you to a separate log search to match on Call-ID and timestamp. Needs `[observability.otlp]` on; export severity is its own `level`. Watch `siphon_ai_otlp_log_records_dropped_total` — movement there means the console has lines the collector never got. |
