...
| `delayed_offer_no_answer.xml`        | The peer **declines** our delayed offer: an offerless INVITE, then an ACK with **no SDP body** (RFC 3261 §13.2.2.4 requires the answer there). The call must be reaped at once as `missing_sdp_answer` — the phase asserts the metric label *and* the CDR cause, and the CDR grep doubles as the timing assertion, since a regression to the Timer-H path writes no CDR for another 32 s (issue #497) (**delayed_offer_no_answer** phase). |
| `rejected_invite_ack.xml`            | An INVITE matching no trunk draws `403`, and the peer ACKs the rejection — what every scanner INVITE on a public listener becomes. That ACK is hop-by-hop (RFC 3261 §17.2.1), so the completed INVITE server transaction must absorb it and the UAS must never see it; the phase asserts the daemon logs no `Received ACK for unknown dialog`. The ACK reuses the INVITE's branch via `[last_Via:]`, which is what makes the transaction match it (siphon-rs#101/#102) (**rejected_invite_ack** phase). |
